...
for row in csvreader:
    ...
    mpaa = 'R'
    if len(plotwords - diffSets['G'] - diffSets['PG-13'] - diffSets['R'] - diffSets['NC-17']) <= 0.5*len(plotwords):
        minimalHits = len(plotwords)/30
        mpaa = 'G'
        if len(diffSets['PG'] & plotwords) > minimalHits:
            mpaa = 'PG'
        if len(diffSets['PG-13'] & plotwords) > minimalHits:
            mpaa = 'PG-13'
        if len(diffSets['R'] & plotwords) > minimalHits:
            mpaa = 'R'
        if len(diffSets['NC-17'] & plotwords) > minimalHits:
            mpaa = 'NC-17'
