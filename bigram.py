...
def train(wordSets, topicSets):
    for age in wordSets.keys():
        doc_clean = ''.join([clean(doc) for doc in wordSets[age]]).split()
        topicSets[age] = FreqDist(nltk.bigrams(doc_clean))
...
for item in topicSets['G'].most_common(2400): #>1 #2400
...
for item in topicSets['PG'].most_common(3700): #>1 #3700
    topicSets['PG-13'].pop(item[0],None)
    topicSets['R'].pop(item[0],None)
    topicSets['NC-17'].pop(item[0],None)
    diffSets['PG'].add(item[0])
for item in topicSets['PG-13'].most_common(3200): #>1 #3200
    topicSets['R'].pop(item[0],None)
    topicSets['NC-17'].pop(item[0],None)
    diffSets['PG-13'].add(item[0]) 
for item in topicSets['R'].most_common(5300): #>1 #5300
    topicSets['NC-17'].pop(item[0],None)
    diffSets['R'].add(item[0])    
for item in topicSets['NC-17'].most_common(2): #>4 #2
    diffSets['NC-17'].add(item[0])    
save_obj(diffSets, 'bigram_diffSets')