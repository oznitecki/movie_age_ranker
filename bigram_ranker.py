...
diffSets = load_obj('bigram_diffSets')
...
for row in csvreader:
    ...
    if len(bigrams & (diffSets['G'] | diffSets['PG-13'] | diffSets['R'] | diffSets['NC-17'])) > 0:
        mpaa = 'G'
        if len(diffSets['PG'] & bigrams) > 0:
            mpaa = 'PG'
        if len(diffSets['PG-13'] & bigrams) > 0:
            mpaa = 'PG-13'
        if len(diffSets['R'] & bigrams) > 0:
            mpaa = 'R'
        if len(diffSets['NC-17'] & bigrams) > 0:
            mpaa = 'NC-17'
    else:
        mpaa = 'R'
