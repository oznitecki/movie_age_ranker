...
def train(wordSets, topicSets):
    for age in wordSets.keys():
        ...
        topicSets[age] = FreqDist(nltk.trigrams(doc_clean))
...
for item in topicSets['G'].most_common(30000): 
...
for item in topicSets['PG'].most_common(45000): 
    topicSets['PG-13'].pop(item[0],None)
    topicSets['R'].pop(item[0],None)
    topicSets['NC-17'].pop(item[0],None)
    diffSets['PG'].add(item[0])
for item in topicSets['PG-13'].most_common(50000): 
    topicSets['R'].pop(item[0],None)
    topicSets['NC-17'].pop(item[0],None)
    diffSets['PG-13'].add(item[0]) 
for item in topicSets['R'].most_common(100000): 
    topicSets['NC-17'].pop(item[0],None)
    diffSets['R'].add(item[0])    
for item in topicSets['NC-17'].most_common(100000): 
    diffSets['NC-17'].add(item[0])    
save_obj(diffSets, 'trigram_diffSets')